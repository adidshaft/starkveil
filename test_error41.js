...
const reqNonce = https.request(rpcUrl, options, (res) => {
    let data = '';
    res.on('data', chunk => data += chunk);
    res.on('end', () => {
        const nonce = JSON.parse(data).result || "0x0";
        ...
        const reqEstimate = https.request(rpcUrl, options, (res) => {
            let data = '';
            res.on('data', chunk => data += chunk);
            res.on('end', () => console.log(JSON.stringify(JSON.parse(data), null, 2)));
        });

        reqEstimate.write(JSON.stringify(payload));
        reqEstimate.end();
    });
});

reqNonce.on('error', e => console.error(e));
reqNonce.write(JSON.stringify(getNoncePayload));
reqNonce.end();
